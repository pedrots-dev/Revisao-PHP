...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { //Processamento de post
    ...
    $personagemCriado = [
        ...
        "nivel" => 1,
        "vida" => (int)($_POST['vidaInicial'] ?? null),
        "isVivo" => true,
        "inventario" => []
    ];
    ...
    if (in_array(null, $personagemCriado, true)) { //validação simples
        ...
        try{
            validarPersonagem($personagemCriado);
                $_SESSION['personagem'] = $personagemCriado;
                $personagemExiste = true;
        } catch (PersonagemException $e) { //Aqui estamos pegando apenas os erros da categoria PersonagemException que foi a categoria criada extendendo o Exception
            echo "Erro na criação do personagem: " . $e->getMessage();
        }
    }
};
